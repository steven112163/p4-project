...
def randomize(version: int, random_mode: int, num_of_switches: int) -> None:
    ...
    with open('p4app.json.txt') as in_file:
        ...
        for i in range(num_of_switches):
            sx = 's' + str(i + 1)
            data['topology']['switches'][sx] = {"cli_input": "./runtime_commands/" + sx + "-commands.txt","program": p4_file_name}
        ...
        if random_mode == 2:
            ...
            for i in range(num_of_switches):
                for j in range(i + 1, num_of_switches):
                    sx1 = 's' + str(i + 1)
                    sx2 = 's' + str(j + 1)
                    links.append([sx1, sx2, {'delay' : f'{randint(0, 100)}ms'}])
        elif random_mode == 0:
            # Equal delay
...
def check_number_range(value: str) -> int:
    ...
    int_value = int(value)
    if int_value < 3:
        raise ArgumentTypeError(f'"{value}" is an invalid random mode. It should be >= 3.')
